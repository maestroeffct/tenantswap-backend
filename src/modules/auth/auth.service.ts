...
export class AuthService {
  ...
  async register(dto: RegisterDto, ip: string) {
    ...
    const user = await this.prisma.user.create({
      data: {
        ...
        workplaceName: dto.workplaceName?.trim() || null,
        workplaceState: dto.workplaceState?.trim() || null,
        workplaceCity: dto.workplaceCity?.trim() || null,
        workplaceArea: dto.workplaceArea?.trim() || null,
  ...
  async login(dto: LoginDto, ip: string) {
    ...
    console.log(`[LOGIN_DEBUG] Who are you? I just logged in: userId=${user.id} phone=${user.phone}`);
